...
//     }
//     await Property.findByIdAndUpdate(propertyId, { rating: parseFloat(avgRating.toFixed(1)) });
// };

const updatePropertyRating = async (propertyRef) => {
    try {
        let propertyId;
        if (propertyRef && propertyRef._id) {
            propertyId = propertyRef._id.toString();
        } else {
            propertyId = propertyRef.toString();
        }
        ...
        console.log(`Updated rating for property: ${propertyId} → ${parseFloat(avgRating.toFixed(1))}`);
    } catch (err) {
...
export const createReview = async (req, res) => {
    ...
    try {
        ...
        await updatePropertyRating(propertyIdToUse);
